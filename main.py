...
Lake_names = ["Newman Lake", "Eloika Lake", "Long Lake", "Silver Lake", "Bonnie Lake", "Sacheen Lake", "Loon Lake", "Liberty Lake"]



def Big_Fish_Pie_Chart(merged_df):
    Big_Fish = merged_df[merged_df["Fish over 3.5lbs"] > 0]
    No_Big_Fish = merged_df[merged_df["Fish over 3.5lbs"] < 1]
    big_fish_len = len(Big_Fish)
    no_big_fish_len = len(No_Big_Fish)
    plt.figure()
    plt.pie([big_fish_len, no_big_fish_len], autopct="%1.1f%%", labels=["Big Fish Caught", "No Big Fish :("])
    plt.show()
for Lake_Name in Lake_names:
    def WT_vs_FC_Plot(lake_name_obj, Lake_Name):
        Lake_Name_df = lake_name_obj.get_group(Lake_Name)
...
def All_Lakes_WT_vs_FC_Plot(lake_name_obj):
    Eloika_Lake_df = lake_name_obj.get_group("Eloika Lake")
    eloika_wt_ser = Eloika_Lake_df["Water Temp"]
    eloika_fc_ser = Eloika_Lake_df["Fish Caught"]
    newman_Lake_df = lake_name_obj.get_group("Newman Lake")
    newman_wt_ser = newman_Lake_df["Water Temp"]
    newman_fc_ser = newman_Lake_df["Fish Caught"]
    long_Lake_df = lake_name_obj.get_group("Long Lake")
    long_wt_ser = long_Lake_df["Water Temp"]
    long_fc_ser = long_Lake_df["Fish Caught"]
    sacheen_Lake_df = lake_name_obj.get_group("Sacheen Lake")
    sacheen_wt_ser = sacheen_Lake_df["Water Temp"]
    sacheen_fc_ser = sacheen_Lake_df["Fish Caught"]
    Loon_Lake_df = lake_name_obj.get_group("Loon Lake")
    loon_wt_ser = Loon_Lake_df["Water Temp"]
    loon_fc_ser = Loon_Lake_df["Fish Caught"]
    liberty_Lake_df = lake_name_obj.get_group("Liberty Lake")
    liberty_wt_ser = liberty_Lake_df["Water Temp"]
    liberty_fc_ser = liberty_Lake_df["Fish Caught"]
    silver_Lake_df = lake_name_obj.get_group("Silver Lake")
    silver_wt_ser = silver_Lake_df["Water Temp"]
    silver_fc_ser = silver_Lake_df["Fish Caught"]
    bonnie_Lake_df = lake_name_obj.get_group("Bonnie Lake")
    bonnie_wt_ser = bonnie_Lake_df["Water Temp"]
    bonnie_fc_ser = bonnie_Lake_df["Fish Caught"]
    plt.figure()
...
def Long_vs_Silver_Test(lake_name_obj):
    long_Lake_df = lake_name_obj.get_group("Long Lake")
    long_fc_ser = long_Lake_df["Fish Caught"]
    silver_Lake_df = lake_name_obj.get_group("Silver Lake")
    silver_fc_ser = silver_Lake_df["Fish Caught"]
...
def Bigs_vs_Pressure_Test(merged_df):
...
def Long_Vs_Silver_Bags_Test(lake_name_obj):
    Silver_Lake_df = lake_name_obj.get_group("Silver Lake")
    silver_f5b_ser = Silver_Lake_df["5 Fish Bag"]
    Long_Lake_df = lake_name_obj.get_group("Long Lake")
    long_5fb_ser = Long_Lake_df["5 Fish Bag"]
...
def Big_Fish_Dec_Tree(X_train_normalized, X_test_normalized, y_train, y_test):

    dec_tree_clf = DecisionTreeClassifier(random_state=0, max_depth=3)
    dec_tree_clf.fit(X_train_normalized, y_train)
...
def Big_Fish_kNN(X_train_normalized, X_test_normalized, y_train, y_test):
